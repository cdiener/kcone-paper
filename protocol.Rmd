...
author: "Christian Diener, Felipe Muñoz, Sergio Encarnación, Osbaldo Resendis"
output:
  html_document:
    highlight: default
    theme: flatly
...
basis vectors each this will take a while. In order to accelerate this, we 
again use the caching operator and will also employ the `doParallel` package
...
```{r}
# Generate hypothesis
h <- hyp(mats[, 1:3], mats[, 4:6], r, type = "fva", obj = prolif, full = T)
pw <- rp(make_irreversible(r), "pathway")[,2]
r_ids <- rp(make_irreversible(r), "KEGG_reaction")[,2]
h$hyp <- cbind(h$hyp, pathway = pw[h$hyp$idx])
h$hyp <- cbind(h$hyp, reaction_id = r_ids[h$hyp$idx])
