...
author: "Christian Diener, Sergio Encarnación, Felipe Muñoz, Osbaldo Resendis"
date: "`r Sys.Date()`"
output:
  html_document:
    highlight: default
...
installed using devtools in the following manner. We will also intall all 
...
irreversible stochiometric matrix is sufficient. However `dycone` uses an 
...
vectors) specifiying the names of substrates and products, the respsective 
stochiometries and the reversibility of the reaction. Each reaction can 
furtermore carry an arbitrary amount of annotations. Those models can be read
...
convert those to micro-mole per litre by using the experimentally quantified 
...
As one can observe, there are several missing measurements (caused if 
metabolite concentrations are below the detection limit) so we will now see how 
...
measurements. For this we will first construct a data frame of all the subtrates
and products in the model along with their KEGG compound IDs.
...
For easier usage we will convert this data frame into a list of unique 
metabolites named by their respective KEGG ID.

```{r}
spec <- spec[!duplicated(spec)]
```

Now we will look for matches of the respective metabolites from the model
in the metabolome measurements. Matching will be done based on KEGG IDs.

```{r}
matches <- sapply(names(spec), grep_or_na, x = metab$kegg_id)
miss <- is.na(matches)
kegg_missing <- names(spec)[miss]
length(kegg_missing)/length(spec)
```

So we see we have measurements for about 2/3 of the required metabolites 
(actually its a bit more since hydrogen was not measured but can be inferred 
from the pH). To impute the missing values we will first construct a data frame 
for all metbaolites with missing entries and see how to fill it in the 
following steps.

```{r}
full <- data.frame(kegg = names(spec), name = spec)
m <- matrix(NA, nrow = nrow(full), ncol = 6)
colnames(m) <- names(metab)[5:10]
full <- cbind(full, m)
matched_idx <- !is.na(matches)
full[matched_idx, 3:8] <- metab[matches[matched_idx], 5:10]
```

`full` now already included the metbaolome measurements but still has missing 
entries which we will no scrape from the Human Metabolome Database (HMDB, 
http://hmdb.ca). Unfortunately, HMDB included some errors in the assignment of 
KEGG identifiers. A revised mapping of KEGG to HMDB identifiers for our model 
can be read from "id_map.csv". For general usage `dycone` also provides the 
`find_hmdb` function which can be used to query HMDB with arbitrary search 
terms.
...
We will now scrape the concentrations for all metbaolites in the model from 
...
We will now get the mean values for measured concentrations (HMDB quantifies
almost all metbaolites by micro-moles per litre as well) by taking them from
...
The concentration for free hydrogen can be obtained from the pH of HeLa cells
measured as 7.34 (http://doi.org/10.1073/pnas.95.12.6803).
...
As described in more detail in Supplementary Text S1 of the publication the
...
will several hours, so we will use the caching operator again to avoid 
recalculating the flux cone everytime we run the analysis. For this we will
need an irreversible stochiometric matrix which can be generated with the
`stochiometry` function of `dycone`.
...
dim(V)
```
The flux cone has >100.000 basis vectors here.

For the metabolic terms we use the mass-action terms generated from the imputated
metbaolome measurements. This is sufficient to create the k-cones for the six
...
To visualize the k-cone we will use the `plot_red` function which first projects
the high-diemnsional k-cone into the two dimension capturing the most variance,
followed by clustering of the extreme rays of the cone to avoid repeatedly using
rays that are very similar. The shaded area corresponds to the interior of the
cone, so all feasible sets of kinetic constants fall into the shaded area of the
respective k-cone. We will use blue for HaCaT cells and red for HeLa. 
Since the used k-means clustering is not entirely deterministic the images here
might look a bit different here than in the publication or be rotated along one
of the two PC axes. However, the overall appearance of the cones should be the
same.

```{r, fig.width=6, fig.height=6}
plot_red(K, col = c(rep(c("blue", "tomato2"), each=3)), r_names=rn)
```
...
plot_red(K_subset, col = c(rep(c("blue", "tomato2"), each=3)), r_names=rn[large])
```

## Stability analysis

To get the stability for an entire k-cone we need to run a stability analysis
for each basis vector of the k-cone. Since we have 6 k-cones with over 100.000
basis vectors each this will take a while. In order to accellerate this, we 
again use the caching opearator and will also employ the `doParallel` package
to run the analysis for each k-cone in parallel. If you have less than 6 CPU
cores adjust the option accordingly or simply do not setup the cluster which
will cause the code to run on a single core.
...
ggplot(stab, aes(x = what, fill = cell_line, group = basis_idx)) + 
	stat_bin(position = "dodge", col = "black") + 
...
the log-transformed metbaolite measurements since the log-transform of the 
...
the empirical distriution of the data. 
...
does that for us. It will generate all log-fold changes between a disease and
normal measurements of metabolic terms and compare those to the log-fold changes
within the normal group as a reference. The output will be sorted by increasing
p-values and mean log-fold changes. We will also use the `full` option to obtain
the raw log-fold changes. We will also append some additional annotations to the 
result using the `rp` function.

```{r}
h <- hyp(mats[, 1:3], mats[, 4:6], r, full = T)
...
`h$hyp` now contains regulatory bias for enzyme activities for each reaction.
...
for compunds required for proliferation. Each of those will be assigned a 
weight which is obtained from the stochiometry of Recon 2 biomass reaction 
...
maximum stochiometry from Recon 2. Negative stochiometries denote compounds
that are consumed and positive stochiometries denote produced compunds. Here
we only produce ADP and Pi to balance the ATP usage. The small difference in 
the stochiometry (20.7045 vs 20.6508) in Recon 2 accounts for the ATP used
...
to the docuemntation of `fba` for alternative input forms.
...
analysis the following code will automatically run in parallel.

```{r}
# Generate hypothesis
h <- hyp(mats[, 1:3], mats[, 4:6], r, type = "fva", obj = prolif, 
    v_min = 1e-6, full = T)
pw <- rp(make_irreversible(r), "pathway")[,2]
r_ids <- rp(make_irreversible(r), "KEGG_reaction")[,2]
h$hyp <- cbind(h$hyp, pathway = pw[h$hyp$idx])
...
```
Let us use visualize those results to also compare the obtained regulations on a
pathway level and mark reactions whose regulatory bias log-fold changes can not
be explained by flux variability.

```{r}
ggplot(h$hyp, aes(y = pathway, x = mean_log_fold, col = pathway)) + 
...
regulatory bias by the maximum log-fold change estimate from the FVA. Thus, marking
reactions whose regulatory bias is essential for growth.
...
```{r}
essential <- abs(h$hyp$mean_log_fold) > h$hyp$fva_log_fold
...
Cancer is known to be a very heterogenous disease. thus, we might ask what
...
and creating an ouptut directory for the downloaded data.
...
coincide in their log fold changes. Significance in regulatory bias and gene
...
that are significant in regulatory bias and gene expression we find some cases
